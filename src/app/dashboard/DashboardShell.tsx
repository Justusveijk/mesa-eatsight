      ...
      <aside className="w-64 bg-ocean-800/80 backdrop-blur-xl border-r border-line flex flex-col">
        ...
        <div className="p-4 border-t border-line">
          <div className="px-4 py-2 mb-2">
            ...
            <p className="text-sm text-text-primary font-medium truncate">{venueName}</p>
            <p className="text-xs text-text-muted mt-1 truncate">{userEmail}</p>
          </div>
          <button
            onClick={handleLogout}
            disabled={isLoggingOut}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-text-muted hover:bg-ocean-700/50 hover:text-text-primary transition-colors disabled:opacity-50"
          >
